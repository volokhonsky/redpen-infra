...
def test_client_key_takes_the_address_the_proxy_appended():
    # Список X-Forwarded-For растёт слева направо, и адрес, дописанный нашим
    # прокси, стоит в конце. Всё, что левее, в общем случае пишет отправитель
    # запроса. Сегодня Caddy оставляет в заголовке один адрес (см. client_key),
    # но чтение с конца верно и тогда, когда их станет несколько.
...
def test_a_forged_header_does_not_buy_a_fresh_bucket():
    """Подделанное начало заголовка не должно снимать предел.

    Сегодня до API такое значение и не доезжает — Caddy заменяет заголовок
    адресом соединения. Проверка держит второй рубеж: она сломается, если
    client_key когда-нибудь снова начнёт читать первый элемент.
    """
