...
def _state(ann_id):
    ...
    row = conn.execute(
        "SELECT category, category_source FROM annotations WHERE ann_id = ?", (ann_id,)
    ).fetchone()
...
def test_decision_is_recorded_even_when_the_value_does_not_change(tmp_path, _fresh_db):
    ...
    """
    db.upsert_annotation_db("doc1", "006", "a1", "main", "пояснение", action="create")
    assert _state("a1") == ("other", "default")

    out = _run(_fresh_db, [{"docId": "doc1", "pageNum": "006", "annId": "a1",
...
def test_real_change_moves_both_value_and_source(tmp_path, _fresh_db):
    db.upsert_annotation_db("doc1", "006", "a2", "main", "текст", action="create")
    _run(_fresh_db, [{"docId": "doc1", "pageNum": "006", "annId": "a2",
...
def test_already_decided_is_left_alone(tmp_path, _fresh_db):
    ...
    db.upsert_annotation_db("doc1", "006", "a3", "main", "текст", action="create",
                            category="other", author_id=1)
    assert _state("a3") == ("other", "human")

    out = _run(_fresh_db, [{"docId": "doc1", "pageNum": "006", "annId": "a3",
...
def test_only_default_skips_what_someone_already_decided(tmp_path, _fresh_db):
    db.upsert_annotation_db("doc1", "006", "a4", "main", "текст", action="create",
                            category="today", author_id=1)
    _run(_fresh_db, [{"docId": "doc1", "pageNum": "006", "annId": "a4",
