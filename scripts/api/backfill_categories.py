#!/usr/bin/env python3
"""Проставить категорию существующим замечаниям.

Категория — своё поле, ровно одно на замечание, по умолчанию `other`
(«Прочее»). Все замечания, заведённые до появления поля, лежат с этим
...
Два источника решений:
  ...
  --from-tags        грубая догадка по имеющимся тегам (таблица приоритетов
                     ...
                     но не как окончательная разметка: у 337 замечаний главный
...
def main() -> int:
    ...
    print(f"замечаний: {len(rows)}, к изменению: {len(changes)}")
