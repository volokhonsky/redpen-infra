"""
CLI: export remarks from the SQLite store (db.py) into the "bare array"
page_NNN.json files used by the portable redpen-publish git snapshot (stage 2,
docs/agent-instructions-stage-2.md, A2.7).

Usage:
    python export_remarks.py --to <dir> [--doc <docId>]

Writes <dir>/<docId>/remarks/page_<NNN>.json for every page that has at
least one remark row, using the same renderer as the live publisher
(publisher.render_page_static) -- published and draft remarks together,
...
def _write_page(target_dir: str, doc_id: str, page_num: str) -> None:
    ...
    out_dir = os.path.join(target_dir, doc_id, "remarks")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--to", dest="target_dir", required=True, help="Directory to write <docId>/remarks/page_*.json into")
