"""Файловый инбокс этапа 0.

ВНИМАНИЕ: модуль обслуживает только `/api/store-raw` и `/api/store` в main.py,
у которых нет ни одного клиента (см. врезку там). Каноном замечаний с этапа 2
является SQLite; сюда ничего не пишется в работе.
"""
