...
готовый инструмент про нас не знает:
  ...
  * выбрасываются запросы рабочего места и опросника (`/work/`, `/survey/`,
    `/api/`, любой адрес с `?editor=1`, а также прежние `/app/` и `/cabinet/`)
    — в аналитику читателей им нельзя;
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--host", action="append", default=[],
                        help="собственный хост: ссылки с него из /work/ и /survey/ отбрасываются")
