...
принадлежит, где разбора ещё нет и какие замечания открывают по прямой ссылке.
...
_RE_PRIVATE = re.compile(r"^/(app|cabinet|survey|work)(/|$)|^/\.hooks/|^/api/")

#: То же самое, но со стороны ссылающейся страницы. `app` и `cabinet` остаются
#: здесь навсегда: исторические access-логи неизменяемы, а в них эти адреса
#: живые (см. CLAUDE.md, «толерантность к прежним именам»).
_RE_PRIVATE_REFERER = re.compile(r"^/(app|cabinet|survey|work)(/|$)")


def private_referer(referer: str, own_hosts: Tuple[str, ...] = ()) -> bool:
    """Запрос сделан со страницы рабочего места или опросника?

    Проверка не косметическая. Предпросмотр в `/work/` грузит в iframe
    **настоящую страницу читателя**, да ещё и с `?only=<id>` (см.
    `templates/js/redpen-preview.js`). По адресу такой запрос неотличим от чтения:
    без этой проверки собственная правка засчитывалась бы как визит, а разбор
    замечания — как пересылку, то есть портил бы единственную метрику,
...
def build_hit(parsed: Dict[str, Any],
    ...
    считается контент — сколько раз открыли страницу, параграф, замечание.
    """
