...
и в шапке рабочего места (`templates/work/index.html`).
...
- API и любые серверные хранилища обслуживают исключительно рабочее место
  (`/work/`) и опросник (`/survey/`);
...
- `templates/` — шаблоны статического сайта
  - `js/` — просмотрщик `page-view.js`, общий модуль маркеров
    ...
    авторизации `redpen-auth.js`, общая конфигурация `redpen-config.js`,
    обёртки над API `redpen-api.js`, предпросмотр читательской страницы во
    фрейме `redpen-preview.js`
  - `work/` — рабочее место: весь разбор и администрирование, экраны по роли
    (параграфы, страница со сканом, сквозной список замечаний, очередь,
    история, опрос, админка)
  - `survey/` — опросник замечаний (открыт кому угодно)
  - `css/`, `favicon.svg`
...
redpen-publish/
  ...
  work/  survey/                   # рабочее место и опросник (не для читателя)
...
2. Замечания пишет агент-аннотатор (`docs/remark-agent-prompt.md`; один
   запуск = один параграф) в `remarks_draft/`, либо люди — в рабочем месте
   `/work/`.
...
4. `build_website.py` собирает всё остальное (шаблоны, картинки, текст, манифест
   страниц, страницы читателя, блог, рабочее место, опросник) в `redpen-publish`;
...
6. Просмотрщик работает только на статике; API обслуживает исключительно
   рабочее место и опросник. Быстрый вход в проект для агентов — корневой
   `CLAUDE.md`.
