...
- Переменные окружения читаются из корневого `.env`: `STORAGE_DIR`,
  `LOG_DIR`, `LOG_LEVEL`, `CORS_ALLOW_ORIGINS`.
- `GET /api/health` → `{"status":"ok"}`
- `POST /api/store` / `POST /api/store-raw` — приём JSON в `${STORAGE_DIR}/inbox/…`
- `GET|POST|PUT /api/editor/{docId}/{pageNum}[/{annId}]` — редактор аннотаций
- `POST /api/rebuild/{bookSlug}/annotations/{pageId}` — Markdown → JSON
