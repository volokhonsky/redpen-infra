...
- запросы к `/work/`, `/survey/`, `/api/`, `/.hooks/` и любой адрес с
  `?editor=1` (плюс прежние `/app/` и `/cabinet/` — фильтр остаётся к ним
  толерантным, исторические логи неизменяемы);
- **запросы со ссылающейся страницы `/work/` или `/survey/`** (и прежних
  `/app/`, `/cabinet/`) — предпросмотр грузит настоящую страницу читателя в
  iframe, ещё и с `?only=<id>`; по адресу это неотличимо от чтения, и без
  такой проверки собственная правка засчитывалась бы как визит, а разбор
  замечания — как пересылку;
