...
## Баг №2 закрыт (2026-08-20)

Фикс выложен на прод (deployment-log, запись 2026-08-20). Причина оказалась
шире одного `handleMarkerClick`: префикс `circle-` попадал в состояние
редактора и из `snapshotDomMarkersToState` (`id: el.id`), из-за чего
`st.page.annotations` не сходился с ответом API по id.

Решение — одна точка нормализации `annIdFromDom()` в
`redpen-editor-bootstrap.js`: id в состоянии и в запросах к API всегда
серверные, `circle-` остаётся деталью DOM. Обязательное дополнение —
симметричный фолбэк на префикс в `markers.upsert()` и в удалении старого узла
при смене id: без него «чистые» id перестали бы находить уже отрисованный
annotations.js узел и рисовали бы второй кружок поверх первого. Точечный обход
в deep-link (`?ann=`) стал не нужен и убран.

Blast radius оказался меньше опасений: сохранение черновиков не затронуто (их
маркеры редактор сеет сам, с чистым id — нормализация для них тождественна).
Дубликатов в БД баг за три недели не наплодил: `SELECT ... WHERE ann_id LIKE
'circle-%'` на проде — 0 строк, уборка не потребовалась.
