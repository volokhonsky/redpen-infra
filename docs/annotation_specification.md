...
## Черновики и режим предпросмотра `?showDrafts=1`

У аннотации в БД есть статус (`published` / `draft` / `deleted`). Черновики
(`status='draft'`) **не попадают** в опубликованный `annotations/page_<NNN>.json`
— он рендерится только из `published`, и его sha используется редактором для
оптимистичной блокировки.

Чтобы черновики можно было посмотреть прямо на статическом сайте, публикатор
(`scripts/api/publisher.py`) на каждую мутацию дополнительно рендерит **отдельный
файл-компаньон** `annotations/page_<NNN>.drafts.json` — голый массив тех же
полей, но с `"draft": true` у каждого элемента. Если черновиков на странице нет,
файл не создаётся (а устаревший — удаляется).

Просмотрщик (`templates/js/main.js`, `isDraftMode()`) загружает этот файл-компаньон
**только** при включённом режиме `?showDrafts=1` (в query или в hash `#showDrafts=1`)
и подмешивает черновики к обычным аннотациям. Черновики рисуются пунктирным
кружком и помечаются `[черновик]` в попапе и в общем блоке. Режим редактора
(`?editor=1`) сюда не входит — он тянет свои живые черновики напрямую из API,
поэтому `isDraftMode()` при `?editor=1` возвращает `false`, чтобы не задваивать
маркеры.

Инвариант статики сохранён: просмотрщик по-прежнему не обращается к API —
черновики берутся из статического файла-компаньона. Учтите, что этот файл
**публично доступен** по прямой ссылке; «скрытость» держится только на незнании
URL и переменной.
