...
**Статус на 2026-08-30: исполнен.** Раздел 0 (снос SPA) выполнен после переезда
редактора в `/app/`, разделы 1–4 закрыты, фаза 6 переименования снята,
документация пересмотрена. Ход работ — `docs/stage-4-progress-log.md`, запись
«Общая уборка». Документ оставлен как обоснование принятых решений; галочки
внутри отражают состояние на день составления, а не на сегодня, и ссылки на
файлы, удалённые при исполнении (`main.js`, `annotations.js`,
`comment-content.js`, `annotation_converter.py`), намеренно не переписаны —
правка сделала бы запись о прошлом ложной.
...
- [x] **1.7** Расселить `.git/modules` (выполнено 2026-08-23). `redpen-content`
      и `redpen-publish` вопреки этому плану оставались физически привязаны к
      инфраструктурному репозиторию: их `.git` были файлами-указателями
      `gitdir: ../.git/modules/<имя>`, а сами репозитории с
      `core.worktree` жили внутри `redpen-infra/.git` — наследие submodules,
      от которых отказались. `rm -rf .git` в `redpen-infra` унёс бы историю
      обеих книг и всего опубликованного сайта (1 ГБ объектов). Каталоги
      перенесены в `redpen-content/.git` и `redpen-publish/.git`,
      `core.worktree` снят, `.git/modules` и секции `[submodule ...]` в
      `.git/config` удалены. `.git` инфры: 1.2 ГБ → 197 МБ.

- [x] **1.8** Переписать историю: выкинуть `tmp/` (выполнено 2026-08-23).
      97.4% истории `redpen-infra` (193.9 МБ из 199 МБ) занимали три снимка
      publish-дерева `tmp/redpen-publish-backup-20250817-*` — 4098 файлов,
      попавших в репозиторий коммитом `fff3061` до того, как появилось правило
      `/tmp/` в `.gitignore`. Правило на уже отслеживаемые файлы не действует,
      поэтому они продолжали жить в `HEAD`. `git filter-repo --path tmp/
      --invert-paths` + force-push: `.git` 193 МБ → 3.7 МБ, рабочая копия
      `tmp/` 787 МБ → 194 МБ. Все 191 коммит сохранены (автор, дата, тема),
      SHA блобов всех 191 отслеживаемого файла не изменились.
      **SHA до этой даты изменились.** Ссылки из `deployment-log.md`
      на коммиты этапа 2 переводятся так:
      `0071460`→`bc65ced`, `cada6d2`→`6be66c1`, `5c55df2`→`d130f40`.
      Старая история целиком лежит в бэкапе
      `~/Documents/redpen-infra-git-backup-20260823-231112` — удалить,
      когда перезапись отстоится.
