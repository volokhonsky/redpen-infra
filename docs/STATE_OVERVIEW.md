...
| `<doc>/document_index.html` | старый SPA; **единственное место, где работает режим редактора** | `layout/comment-content/annotations/mobile/main` + редакторские |
...
- Режим редактора (включается `?editor=1` или глобальным флагом):
  ...
  - Типы аннотаций — только `main` и `comment`; координаты обязательны для обоих. Тип `general` (общий комментарий страницы) упразднён миграцией 2026-08-16: конвертер отвергает его с ошибкой (`annotation_converter.py`), в селекторе панели его нет (`redpen-editor-panel.js`). Остатки обработки `general` в `redpen-editor-bootstrap.js` — легаси для чтения старых черновиков.
