...
Лимит Pro обрывает прогон в произвольный момент, а время сброса заранее неизвестно
(в логе иногда есть подсказка вида `resets 12pm (UTC)`, иногда нет). Поэтому раннер
не сдаётся, а **сам пересобирает контейнер каждые 20 минут и продолжает** — до
24 попыток (≈8 часов ожидания). Переопределяется переменными окружения:

```bash
RETRY_INTERVAL=600 MAX_ATTEMPTS=6 zsh ~/.claude-agent-docker/run-agent.sh para25 ~/.claude-agent-docker/para25-prompt.txt
```
...
- **Продолжение, а не переделка.** Состояние `claude` вынесено из контейнера на хост:
  ...
  перезапуск, и повторная попытка идёт с `claude --continue` — с преамбулой «сессия
  оборвалась по лимиту, проверь, что уже сделано на диске, готовое не переделывай»
  плюс исходное задание целиком.
